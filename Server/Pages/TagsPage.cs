...
record TagsPageBodyContent : PageFragment
{
    public override async Task<Html> RenderAsync(IServiceProvider serviceProvider)
    {
        ...
        var tags = await db.Tags
            .AsNoTracking()
            .Select(t => new TagListItem(t.Id, t.Name, t.Inputs.Count))
            .ToArrayAsync();
        
        Html RenderExistingTag(TagListItem t) => new($"""
    ...
    record TagListItem(int Id, string Name, int InputCount)
    {
        public string Color() => Id.Color();
    }
}