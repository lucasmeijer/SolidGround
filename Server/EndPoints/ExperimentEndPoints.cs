...
static class ExperimentEndPoints
{
    ...
    public static void MapExperimentEndPoints(this IEndpointRouteBuilder app)
    {
        ...
        app.MapPost(Routes.api_experiment, async (AppDbContext db, IConfiguration config, IServiceProvider serviceProvider, HttpRequest request) =>
        {
            ...
            var inputIds = (await db.Inputs.ToArrayAsync()).Select(i => i.Id).ToArray();
            ...
            foreach (var (inputId, output) in inputsToOutputs)
            {
                ...
                _ = Task.Run(() => ExecutionForInput(inputId, output, appEndPoint, variables, serviceProvider));
            }
            
            return new TurboStreamCollection(inputIds.Select(i => new TurboStream("replace",TurboFrameContent:new InputTurboFrame(i))).ToArray());
            ...
            Output OutputFor(int inputId) => new()
            {
                ...
                StringVariables =
                [
                    ..variables.Select(kvp => new StringVariable
                        { Name = kvp.Key[prefix.Length..], Value = kvp.Value })
                ],
    ...
    static async Task<Input> LastInput(AppDbContext db)
    {
        return await db.Inputs.OrderByDescending(i => i.Id).FirstAsync() ??
               throw new BadHttpRequestException("No inputs");
    }

    static async Task ExecutionForInput(int inputId, Output output, string appEndPoint, Dictionary<string, string> variables, IServiceProvider serviceProvider)
    {
